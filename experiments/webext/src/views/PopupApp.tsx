  ...
  useEffect(() => {
    async function createCardFromCurrentTab() {
      ...
      if (!settings.host || !settings.token) {
        setError(
          "Looks like you're unauthenticated. Please configure the extension's settings first."
        );
        return;
      }
      if (!settings.boardId) {
        setError(
          "Looks like you haven't selected a board to save to yet. Please configure the extension's settings first."
        );
        return;
      }
      setInProgress(true);
      try {
        ...
        const urlPropertyId = await findUrlPropertyId(
          settings.host as string,
          settings.token as string,
          settings.boardId as string
        );
        await createCard(
          settings.host as string,
          settings.token as string,
  ...
  return (
    <div className="PopupApp">
      <div className="status">
        {inProgress && (
          <div className="in-progress">Saving to Karamalsharq...</div>
        )}
        {!inProgress && !error && (
